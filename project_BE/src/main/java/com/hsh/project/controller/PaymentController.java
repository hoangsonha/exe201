...
public class PaymentController {
    ...
    public ResponseEntity<ObjectResponse> initiatePayment(
        ...
        try {
            ...
            // Use VNPayConfig to build payment URL
            ...
            params.put("vnp_Amount", String.valueOf((int) (amount * 100))); // Convert to VND smallest unit
            params.put("vnp_CurrCode", "VND");
            params.put("vnp_TxnRef", userId + "_" + VNPayConfig.getCurrentDateTime()); // Unique transaction reference
            params.put("vnp_OrderInfo", orderInfo);
            params.put("vnp_OrderType", VNPayConfig.ORDER_TYPE);
            params.put("vnp_Locale", "vn");
            params.put("vnp_ReturnUrl", returnUrl != null ? returnUrl : VNPayConfig.VNPAY_RETURN_URL);
            ...
            // Save payment record (assuming Payment entity and repository)
            Payment payment = new Payment();
            payment.setUser(user); // Assuming userId is stored as Long in Payment
            payment.setAmount(amount);
            payment.setStatus(EnumPaymentStatus.PENDING);
            payment.setTransactionId(params.get("vnp_TxnRef"));
            // Assuming paymentRepository is injected; adjust if different
