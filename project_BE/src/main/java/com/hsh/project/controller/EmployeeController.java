...
public class EmployeeController {
    ...
    @GetMapping("")
    public ResponseEntity<PagingResponse> getAllPurposes(
    ...
    public ResponseEntity<ObjectResponse> getAllAccountsNonPaging() {
        List<UserDTO> results = employeeService.getAccounts();
        return !results.isEmpty()
                ? ResponseEntity.status(HttpStatus.OK)
                        .body(new ObjectResponse("Success", "get all hub non paging successfully", results))
                : ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ObjectResponse("Failed", "get all hub non paging failed", results));
    }

    @PreAuthorize("hasRole('USER') or hasRole('ADMIN')")
    @GetMapping("/search")
    public ResponseEntity<PagingResponse> searchVaccines(
            @RequestParam(value = "currentPage", required = false) Integer currentPage,
            @RequestParam(value = "pageSize", required = false) Integer pageSize,
        ...
        PagingResponse results = employeeService.searchEmployees(resolvedCurrentPage, resolvedPageSize, userName,
                fullName, email);
        List<?> data = (List<?>) results.getData();
        return ResponseEntity.status(!data.isEmpty() ? HttpStatus.OK : HttpStatus.BAD_REQUEST).body(results);
    }
    ...
    public ResponseEntity<ObjectResponse> getHubByID(@PathVariable("id") int id) {
        UserDTO hub = employeeService.getAccountById(id);
        return hub != null
                ? ResponseEntity.status(HttpStatus.OK)
                        .body(new ObjectResponse("Success", "Get hub by ID successfully", hub))
                : ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ObjectResponse("Fail", "Get hub by ID failed", null));
    }
    ...
    public ResponseEntity<ObjectResponse> createEmployee(@Valid @RequestBody CreateEmployeeRequest req) {
        try {
            UserDTO employee = employeeService.createEmployee(req);
            return ResponseEntity.status(HttpStatus.OK)
                    .body(new ObjectResponse("Success", "Create hub successfully", employee));
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ObjectResponse("Fail", "Create hub failed", null));
        }
    }

    @PreAuthorize("hasRole('MANAGER') or hasRole('ADMIN')")
    @PutMapping("/{id}")
    public ResponseEntity<ObjectResponse> updateHub(@PathVariable("id") int id,
            @RequestBody UpdateEmployeeRequest req) {
        try {
            UserDTO employee = employeeService.updateEmployee(req, id);
            if (employee != null) {
                return ResponseEntity.status(HttpStatus.OK)
                        .body(new ObjectResponse("Success", "Update hub successfully", employee));
            }
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ObjectResponse("Fail", "Update hub failed. Hub is null", null));
        } catch (BadRequestException e) {
            log.error("Error creating hub", e);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ObjectResponse("Fail", e.getMessage(), null));
        } catch (ElementExistException e) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ObjectResponse("Fail", "Update hub failed", null));
        }
    }
    ...
    public ResponseEntity<ObjectResponse> deleteHubByID(@PathVariable("id") int hubID) {
        try {
            User hub = employeeService.getEmployeeById(hubID);
            if (hub != null) {
                hub.setDeleted(true);
                hub.setEnabled(false);
                return ResponseEntity.status(HttpStatus.OK).body(
                        new ObjectResponse("Success", "Delete hub successfully", employeeService.saveEmployee(hub)));
            }
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ObjectResponse("Fail", "Delete hub failed", null));
        } catch (Exception e) {
    ...
    public ResponseEntity<ObjectResponse> unDeleteHubByID(@PathVariable("id") int id) {
        try {
            User hub = employeeService.getEmployeeById(id);
            if (hub != null) {
                hub.setDeleted(false);
                hub.setEnabled(true);
                return ResponseEntity.status(HttpStatus.OK).body(
                        new ObjectResponse("Success", "UnDelete hub successfully", employeeService.saveEmployee(hub)));
            }
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ObjectResponse("Fail", "Employee is null", null));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ObjectResponse("Fail", "Undelete hub failed", null));
        }
    }
